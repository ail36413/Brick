package com.ail.brick.net.http.util

import com.ail.brick.net.http.model.IBaseResponse
import com.ail.brick.net.http.model.NetworkResult
import com.ail.brick.net.http.model.ProgressInfo
 ...
 */
@Singleton
@Suppress("unused", "MemberVisibilityCanBePrivate") // Public API — methods are called by app modules via DI
class NetworkExecutor @Inject constructor(
     ...
     * @param retryDelayMs 重试间隔毫秒数，默认 300ms
     */
    @Suppress("unused") // Public API — convenience method for app modules
     ...
     * @param lifecycleScope 可选：在指定协程域内执行以便与生命周期绑定
     * @param tag 可选监控标签
    ...
    suspend fun downloadFile(
        ...
        return downloadExecutor.downloadFile(targetFile, progressFlow, expectedHash, hashAlgorithm, hashStrategy, cancelJob, lifecycleScope, tag, call)
    }
     ...
     */
    @Suppress("unused") // Public API — used by app modules for multipart upload parts
     ...
     */
    @Suppress("unused") // Public API — single file upload shortcut
    suspend fun <T> uploadFile(
        ...
        return uploadExecutor.uploadFile(file, partName, progressFlow, successCode, dispatcher, lifecycleScope, tag, call)
    }
     ...
     */
    @Suppress("unused") // Public API — multi-part upload interface
    suspend fun <T> uploadParts(
        ...
        return uploadExecutor.uploadParts(parts, formFields, successCode, dispatcher, lifecycleScope, tag, call)
    }
}
