...
import com.ail.brick.net.http.exception.ExceptionHandle
import com.ail.brick.net.http.model.IBaseResponse
import com.ail.brick.net.http.model.NetCode
import com.ail.brick.net.http.model.NetEvent
import com.ail.brick.net.http.model.NetEventStage
 ...
 *
 * @param configProvider 提供当前 [com.ail.brick.net.http.annotations.NetworkConfig] 实例
...
class RequestExecutor @Inject constructor(
     ...
     *
     * 协程级重试与 OkHttp 的 [com.ail.brick.net.http.interceptor.DynamicRetryInterceptor] 不同：
    ...
    suspend fun <T> executeRequest(
        ...
        val finalResult = if (result is NetworkResult.BusinessFailure && result.code == NetCode.Biz.UNAUTHORIZED) {
            if (!tokenProviderOptional.isPresent) {
                ...
                refreshMutex.withLock {
                    ...
                    // Step 1: Retry — another coroutine may have already refreshed the token while we waited for the lock
                    ...
                    // Step 2: Still 401 — perform actual token refresh
                    val refreshed = try {
                    ...
                    // Step 3: Retry once with the refreshed token
                    try {
        ...
        val finalResult = lastResult as NetworkResult<T>
