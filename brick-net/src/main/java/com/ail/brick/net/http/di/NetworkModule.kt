...
import com.ail.brick.net.http.auth.TokenAuthenticator
import com.ail.brick.net.http.auth.TokenProvider
import com.ail.brick.net.http.interceptor.DynamicBaseUrlInterceptor
import com.ail.brick.net.http.interceptor.DynamicTimeoutInterceptor
import com.ail.brick.net.http.interceptor.ExtraHeadersInterceptor
import com.ail.brick.net.http.interceptor.PrettyNetLogger
import com.ail.brick.net.http.interceptor.DynamicPollingInterceptor
import com.ail.brick.net.http.model.GlobalResponseTypeAdapterFactory
import com.ail.brick.net.http.interceptor.DynamicRetryInterceptor
import com.ail.brick.net.http.util.DefaultRetryStrategy
import com.ail.brick.net.http.util.NetworkClientFactory
import com.ail.brick.net.http.util.orDefault
import com.ail.brick.net.http.util.getOrNull
...
object NetworkModule {
    ...
    fun provideOkHttpClient(
        ...
        val builder = OkHttpClient.Builder()
            ...
            // 拦截器执行顺序说明（应用拦截器）：
            // 1. 动态轮询（@Polling）：最外层，始终执行 N 次
            // 2. 动态 BaseUrl：尽早确定最终 host/schema/port
            // 3. 动态超时：基于注解覆写本次请求的超时配置
            // 4. 公共头：在 URL/超时都确定之后，补齐通用 Header
    ...
    private fun resolveHttpLogLevel(config: com.ail.brick.net.http.annotations.NetworkConfig): HttpLoggingInterceptor.Level {
