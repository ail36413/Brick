...
object BrickStartup {
    ...
    private const val TAG = "BrickStartup"
    ...
    private fun createBackgroundExecutor(): ExecutorService {
        return Executors.newFixedThreadPool(2) { r ->
            Thread(r, "BrickStartup-BG").apply { isDaemon = true }
        }
    }
    ...
    fun register(initializer: AppInitializer) {
        check(!started.get()) { "BrickStartup 已启动，无法再注册初始化器" }
